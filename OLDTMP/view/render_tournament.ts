...
let currentMode: "local" | "remote" = "remote";

export async function render_tournament(params: URLSearchParams | null, mode: "remote" | "local" = "remote") {
	...
	currentMode = mode;
	console.log("Rendering tournament view in mode:", currentMode);
	const toggleHtml = `
		<div id="toggleBtn" class="absolute top-24 right-4 flex hidden items-center gap-2">
...
function renderLocalTournamentFrontend(tournament: any) {
	const html = `
		<div class="flex flex-col items-center gap-8 p-8">
						...
						return `
							<div class="bg-gray-800 rounded-lg p-4 flex flex-col items-center gap-2 text-white">
								...
								<img src="/api/get/getImage?filename=${encodeURIComponent(p.path || "std_user_img.png")}"
									 class="w-24 h-24 rounded-full object-cover">
								<input type="text"
									   placeholder=${t('game.tournament.alias')}
									   class="mt-2 px-2 py-1 rounded bg-gray-600 text-white w-24 text-center"
									   id="player${i + 1}Input"
									   value="${p.username && p.username !== `Player ${i + 1}` ? p.username : ''}">
			...
			<button id="startLocalTournamentBtn" disabled
				class="mt-8 px-6 py-3 bg-green-600 hover:bg-green-700 text-white font-bold rounded-lg transition">
			...
			<div class="grid grid-cols-2 md:grid-cols-4 gap-6 w-full max-w-5xl">
				${players.map(player => {
					if (player.id && player.status !== "left") {
						const statusText =
							player.status === "joined" ? "Joined" :
							player.status === "invited" ? "Waiting for acceptance..." : "";
						const statusClass =
							player.status === "invited" ? "text-yellow-400 animate-pulse" :
