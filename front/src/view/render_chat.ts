...
export async function render_chat(params: URLSearchParams | null) {
  ...
  <div class="chat flex gap-6 w-full h-full max-h-[calc(100vh-8rem)] bg-gradient-to-br from-[#1a1a3d] to-[#252565] rounded-xl shadow-xl p-4">
    ...
    <div class="sidebar w-64 bg-[#0f0f2a] rounded-lg p-3 flex flex-col">
      <h3 class="text-white font-bold text-lg mb-3">${t(lang.friends, LANGUAGE)}</h3>
    ...
    <div class="chat-window flex-1 flex flex-col bg-[#0f0f2a] rounded-lg p-3 overflow-hidden">
      <div class="flex justify-between items-center mb-3">
        <div id="chatHeader" class="font-bold text-white text-lg">${t(lang.selectChatPartner, LANGUAGE)}</div>
      ...
      <div class="mt-3 flex gap-2">
        <input id="chatInput" type="text" placeholder="${t(lang.EnterMessage, LANGUAGE)}" class="flex-1 p-2 rounded-lg border border-gray-600 bg-[#2c2c58] text-white focus:outline-none focus:ring-2 focus:ring-[#5656aa]">
        <button id="sendBtn" class="px-4 py-2 rounded-lg bg-[#5656aa] text-white font-semibold hover:bg-[#7878cc] transition">${t(lang.send, LANGUAGE)}</button>
      </div>
    </div>
  </div>
`;
