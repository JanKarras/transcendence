...
export async function render_two_fa(params: URLSearchParams | null) {
    ...
    `;
		...
		submit?.addEventListener("click", () => two_fa(email));

	} else if (method === 'authapp') {
		// --- Authenticator App 2FA ---
		bodyContainer.innerHTML = `
      <div id="authAppValidationContainer" class="max-w-md p-6 bg-white rounded-lg shadow-lg text-center">
        <h2 class="text-2xl font-bold mb-6 text-gray-800">${t('authAppTitle')}</h2>
        <p class="mb-4 text-gray-700">
          ${t('authAppInstruction')} <span class="font-semibold">${email}</span>
        </p>
        <input type="text" id="authAppCode" inputmode="numeric" maxlength="6" pattern="[0-9]{6}"
