...
export async function render_profile_settings(params: URLSearchParams | null) {
	...
	inputs.forEach(input => {
		if (input.type === 'file') return; // File später behandeln
...
export function renderTwoFASettings(user: UserInfo, backToProfile: () => void): void {
	...
	async function updateMethodStyles() {
		...
		if (selected === 'email') {
			if (emailLabelText) emailLabelText.classList.remove('opacity-50');
			if (appLabelText) appLabelText.classList.add('opacity-50');
			// QR-Code verstecken
			if (qrContainer) qrContainer.classList.add('hidden');
		} else if (selected === 'authapp') {
			if (appLabelText) appLabelText.classList.remove('opacity-50');
			if (emailLabelText) emailLabelText.classList.add('opacity-50');
	...
	toggle?.addEventListener('change', () => {
		if (toggle.checked) {
			...
			if (qrContainer) qrContainer.classList.add('hidden'); // QR bei Deaktivierung ausblenden
		}
		updateMethodStyles();
	});
	...
	saveBtn?.addEventListener('click', async () => {
		...
		const selected = (document.querySelector('input[name="twofaMethod"]:checked') as HTMLInputElement)?.value || 'email';
