...
export async function showFriendsDropdown() {
    ...
    for (const friend of friends) {
        const lastSeen = friend.last_seen
            ? new Date(friend.last_seen + ' UTC').getTime()
            : 0;
    ...
    if (friends.length === 0) {
        ...
        [...onlineFriends, ...offlineFriends].forEach(async (friend) => {
            ...
            listContainer.appendChild(renderFriendItem(friend, onlineFriends.includes(friend), hasUnread));
        });
    }
...
function renderFriendItem(friend, isOnline, hasUnread) {
